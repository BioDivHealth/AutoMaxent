...
Auto_maxent<-function(
        ...
        field_date = "eventDate", #[CHARACTER] Field in the species records contaning the date information
        time_units = "year", # [CHARACTER] main time units in which time-lags are measured
        time_lag = 1, # [NUMERIC] the time-lag to aggregate the environmental data
        id.point = "gbifID", # [CHARACTER] Identifier of the species record
        continuous_data = "9999-01-01", # [CHARACTER] Date, in the data cube object, identifying the 
  ...
  # Some adjustments for the spatial calcualtions
  sf::sf_use_s2(FALSE)
  ...
  if(mod.select==TRUE){
    ...
    if(time_macth == TRUE){
      ...
      print("Running MaxEnt SDM with time macthing environmental processing.")
  ...
  if("SpatRaster" %in% class(predictors)){
    pred.dat <- predictors %>% terra::crop(y=sty.a %>% vect(),mask=TRUE,tempdir=tempdir())
    gc():gc()
  }else{
      stop(" predictors data is not an SpatRaster object!")
    }
  ...
  if(time_macth == TRUE){
    raw.time <- Time_matchine(x = pred.dat, y = y_points, y.t = field_date, 
                              time.i = time_units, 
                              jump.i = time_lag, 
                              id.p = id.point,
                              ...
                              bk_sampling = TRUE, 
                              sampling_type = type_bk, 
                              sampling_area = sty.a, 
                              number_points = n_bk/2) %>% as.data.frame()
    
    # Get the vector of presence and absences
    obs_index <- raw.time[,colnames(raw.time) %in% c("presence")]
    
    # Select the time-period to run predictions
    obs_time <- xtabs(~raw.time$time_step) %>% as.data.frame()
    mod.dat <- raw.time[,!colnames(raw.time) %in% c("X","Y","ID.x","time_step","ID","presence")]
    
    #
    x.tt <- raw.time[nrow(raw.time),"time_step"] ; x.tt<-x.tt %>% strsplit(split=":")
    
      if(sum(time(pred.dat) == continuous_data)==0){
        pred.dat <- pred.dat[[time(pred.dat) >= x.tt[[1]][1] & time(pred.dat) < x.tt[[1]][2]]]
      }else{
        c.var <- pred.dat[[time(pred.dat) == continuous_data]] 
        pred.dat <- pred.dat[[time(pred.dat) >= x.tt[[1]][1] & time(pred.dat) < x.tt[[1]][2]]]
      
        pred.dat <- c(pred.dat,c.var)
      }
    }
  ...
  for(w in 1:n.m){
    ...
    y.data <- data.frame(mod.id = w,
                         ...
                         n_background = if(time_macth == FALSE){bk_points %>% nrow()}else{c(raw.time$presence==0) %>% sum()},
                         bk_type=type_bk,
                         Test_prop = Test_n,
  ...
  if(mod.select==TRUE){  
    ...
    if(length(select_models)==0){
      return(list(Species=name.mod,AIC_mods=AICc_mods_raw,mod_params=mod.params))
  ...
  if(return.all==TRUE){
    ...
    xp<-list(mods=models,
             mod.data = mod.dat,
             time.data = if(time_macth==TRUE){raw.time}else{NULL},
             params=mod.params,
             AICc=AICc_mods,
             ARGS=mod.ARGS,
             variables=vars_s,
